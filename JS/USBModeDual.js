...
/* ENABLE AND DISABLE NORMAL FUNCTIONS (FEATURES, SHARING...) */
...
function enableUsbModeDual() {
  ...
  disableNormalFunctions();
  usbmode_enabled();

  xapi.Command.UserInterface.Extensions.Panel.Update({
    PanelId: PANELID,
    Name: 'Désactiver Comodal écrans étendus (USB)'
  });
  ...
  xapi.Config.Audio.Input.HDMI[localPcInput2].VideoAssociation.MuteOnInactiveVideo.set('Off');
...
function disableUsbModeDual() {
  ...
  xapi.Command.UserInterface.Extensions.Panel.Update({
    PanelId: PANELID,
    Name: 'Activer Comodal écrans étendus (USB)'
  });

  /* CONFIGURE HDMI INPUTS */
  xapi.Config.Audio.Input.HDMI[localPcInput1].VideoAssociation.MuteOnInactiveVideo.set('On');
...
function tvOn(force) {
  Rkhelper.IMC.callFunction('tvOn', force);
}
function tvOff(force) {
  Rkhelper.IMC.callFunction('tvOff', force);
}
function projOn(force) {
  Rkhelper.IMC.callFunction('projOn', force);
}
function projOff(force) {
  Rkhelper.IMC.callFunction('projOff', force);
}
function screenUp(force) {
  Rkhelper.IMC.callFunction('screenUp', force);
}
